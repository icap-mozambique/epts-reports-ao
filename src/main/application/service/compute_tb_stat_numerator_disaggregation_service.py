from logging import Logger
import pandas as pd
from src.main.application.income import ComputeTbStatNumeratorDisaggregationUseCase
from src.main.application.out import IndicatorMetadataPort

class ComputeTbStatNumeratorDisaggregationService(ComputeTbStatNumeratorDisaggregationUseCase):

    def __init__(self, logger: Logger, tb_stat_numerator_indicator_metadata_port: IndicatorMetadataPort) -> None:
       self.logger = logger
       self.tb_stat_numerator_indicator_metadata_port = tb_stat_numerator_indicator_metadata_port

    def compute(self, tb_stat_patients, end_period):
        
        indicators = {}

        indicators_metadata = self.tb_stat_numerator_indicator_metadata_port.find_indicator_metadata()

        for patient in tb_stat_patients:

            for gender in self.GENDERS:
                
                if not self.match_gender(patient, gender):
                    continue

                for result in self.RESULTS:

                    if not self.match_result(patient, result):
                        continue

                    for age_band in self.tb_stat_numerator_indicator_metadata_port.age_bands():

                        if not self.match_age_band(patient, age_band, end_period):
                            continue

                        indicator_key = age_band +'_'+gender[0] + '_' + result
                        metadatas = [indicator_metadata for indicator_metadata in indicators_metadata if indicator_key == indicator_metadata['indicator_key']]

                        # assure facility disaggregation
                        indicator_key = indicator_key + '_' + patient['orgUnit']

                        if indicator_key not in indicators:
                            indicators[indicator_key] = {'indicator_key': indicator_key, 'value':1}
                            metadata = metadatas[0]
                        
                            indicators[indicator_key]['dataElement'] = metadata['id'].split('.')[0]
                            indicators[indicator_key]['categoryOptionCombo'] = metadata['id'].split('.')[1]
                            indicators[indicator_key]['attributeOptionCombo'] = ''
                            indicators[indicator_key]['orgUnit'] = patient['orgUnit']

                        else:
                            indicators[indicator_key]['value'] = indicators[indicator_key]['value'] + 1
                    
                    break
        
        indicators = list(indicators.values())

        return indicators

    def match_gender(self,patient, gender):
        if patient['patientSex'][0] == gender[0]:
            return True
        
        return False
    
    def match_age_band(self, patient, age_band, end_period):
        end_period = pd.to_datetime(end_period)
        
        date_of_birth = pd.to_datetime(patient['patientAge'])
        
        years_between = end_period.year - date_of_birth.year

        if age_band == self.LESS_THAN_ONE_YEAR and years_between < 1:
            return True
        
        if age_band == self.FIFTY_MORE and years_between >= 50:
            return True
        
        if age_band != self.LESS_THAN_ONE_YEAR and age_band != self.FIFTY_MORE:
            start_range = int(age_band.split('-')[0])
            end_range = int(age_band.split('-')[1])

            if (years_between >= start_range and years_between <= end_range):
                return True
            
        return False
    
    def match_result(self, patient, result):
        if patient['testResult'] == 'POSITIVO_CONHECIDO' and result == 'Known at Entry Positive':
            return True
        
        if patient['testResult'] == 'POSITIVO' and result == 'Newly Identified Positive':
            return True
        
        if patient['testResult'] == 'NEGATIVO' and result == 'Newly Identified Negative':
            return True
        
        return False
