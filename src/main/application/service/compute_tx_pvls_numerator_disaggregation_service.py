import pandas as pd
from src.main.application.income import ComputeTxPlvsNumeratorDisaggregationUseCase
from src.main.application.out import IndicatorMetadataPort

class ComputeTxPvlsNumeratorDisaggregationService(ComputeTxPlvsNumeratorDisaggregationUseCase):

    def __init__(self, tx_pvls_numerator_indicator_metadata_port: IndicatorMetadataPort, tx_pvls_numerator_preg_breast_indicator_metadata_port: IndicatorMetadataPort) -> None:
        self.tx_pvls_numerator_indicator_metadata_port = tx_pvls_numerator_indicator_metadata_port
        self.tx_pvls_numerator_preg_breast_indicator_metadata_port = tx_pvls_numerator_preg_breast_indicator_metadata_port

    def compute(self, tx_pvls_patients, end_period):

        tx_pvls_numerator_indicator_metadata = self.tx_pvls_numerator_indicator_metadata_port.find_indicator_metadata()
        tx_pvls_numerator_preg_breast_inidcator_metadata = self.tx_pvls_numerator_preg_breast_indicator_metadata_port.find_indicator_metadata()
        ...
        for patient in tx_pvls_patients:
            ...
            for gender in self.GENDERS:
                ...
                for age_band in self.tx_pvls_numerator_indicator_metadata_port.age_bands():
                    
                    if not self.match_age_band(patient, age_band, end_period):
                        continue

                    indicator_key = age_band+'_'+gender[0]
                    self.update_indicator_value(patient, indicators, tx_pvls_numerator_indicator_metadata, indicator_key)

                    if self.pregnant(patient):
                        indicator_key = 'pregnant'
                        self.update_indicator_value(patient, indicators, tx_pvls_numerator_preg_breast_inidcator_metadata, indicator_key)
