...
import pandas as pd
from src.main.application.income import ComputeTbStatDenominatorDisaggregationUseCase
from src.main.application.out import IndicatorMetadataPort

class ComputeTbStatDenominatorDisaggregationService(ComputeTbStatDenominatorDisaggregationUseCase):

    def __init__(self, logger: Logger, tb_stat_denominator_indicator_metadata_port: IndicatorMetadataPort) -> None:
       self.logger = logger
       self.tb_stat_denominator_indicator_metadata_port = tb_stat_denominator_indicator_metadata_port

    def compute(self, tb_stat_patients, end_period):
        
        indicators = {}

        indicators_metadata = self.tb_stat_denominator_indicator_metadata_port.find_indicator_metadata()
        ...
        for patient in tb_stat_patients:
            ...
            for gender in self.GENDERS:
                ...
                for age_band in self.tb_stat_denominator_indicator_metadata_port.age_bands():
